...
const SERVER_DIR = path.resolve('E:/emran vai/server');

const files = {
  'src/models/Borrow.js': `const mongoose = require('mongoose');
...
`,
  ...
  'src/controllers/borrowController.js': `const Book = require('../models/Book');
...
`,
  ...
  'src/routes/borrowRoutes.js': `const express = require('express');
...
`,
  ...
  'src/controllers/statsController.js': `const Book = require('../models/Book');
const Borrow = require('../models/Borrow');
const { checkDb } = require('../config/db');
...
for (const [relPath, content] of Object.entries(files)) {
  const fullPath = path.join(SERVER_DIR, relPath);
  fs.mkdirSync(path.dirname(fullPath), { recursive: true });
  fs.writeFileSync(fullPath, content, 'utf8');
}
console.log('Backend return request workflow files updated successfully.');
